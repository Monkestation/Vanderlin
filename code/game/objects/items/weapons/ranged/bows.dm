...
/obj/item/gun/ballistic/revolver/grenadelauncher/bow/process_fire(atom/target, mob/living/user, message = TRUE, list/modifiers, zone_override, bonus_spread = 0)
	...
	for(var/obj/item/ammo_casing/CB in get_ammo_list(FALSE, TRUE))
		...
		if(user.client.chargedprog < 100)
			loaded_projectile.damage *= (user.client.chargedprog / 100)
		else
			loaded_projectile.damage = loaded_projectile.damage
			loaded_projectile.embedchance = 100
			loaded_projectile.accuracy += 15 //fully aiming bow makes your accuracy better.
		...
		loaded_projectile.damage *= damfactor // Apply bow's inherent damage multiplier regardless of PER
