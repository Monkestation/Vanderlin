...
/obj/item/gun/ballistic/revolver/grenadelauncher/airgun/process_fire(atom/target, mob/living/user, message = TRUE, params = null, zone_override = "", bonus_spread = 0)
	...
	for(var/obj/item/ammo_casing/CB in get_ammo_list(FALSE, TRUE))
		...
		if(perception > 8)
			loaded_projectile.accuracy += (perception - 10) * 2
			loaded_projectile.bonus_accuracy += (perception - 10)
		...
		loaded_projectile.bonus_accuracy += (GET_MOB_SKILL_VALUE_OLD(user, /datum/attribute/skill/craft/engineering) * 4)
