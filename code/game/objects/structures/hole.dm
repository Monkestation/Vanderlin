...
/obj/structure/closet/dirthole/closed
	...
	var/decor_quality = 1
	/// Decor with patrons supported NOT in list, is skipped. If empty, only general headstones are allowed.
	var/list/patrons_allowed = list()

	/// If set to TRUE, picks a random Headstone, factors to affect this done in `Initialize()`
	var/spawn_headstone = TRUE
	/// If unset, will be generated by `Initialize()`
...
/obj/structure/closet/dirthole/closed/t2
	decor_quality = 2
	spawn_gravefence = TRUE

/obj/structure/closet/dirthole/closed/t3
	decor_quality = 3

/// Tennite Graves
/obj/structure/closet/dirthole/closed/tennite
	patrons_allowed = UNDIVIDED_TEMPLE_PATRONS

/obj/structure/closet/dirthole/closed/tennite/t2
	decor_quality = 2
	spawn_gravefence = TRUE

/obj/structure/closet/dirthole/closed/tennite/t3
	decor_quality = 3

/// Psydonite Graves (Should not be in a Tennite church)
/obj/structure/closet/dirthole/closed/psydon
	patrons_allowed = list(/datum/patron/psydon, /datum/patron/psydon/extremist)

/obj/structure/closet/dirthole/closed/psydon/t2
	decor_quality = 2
	spawn_gravefence = TRUE

/obj/structure/closet/dirthole/closed/psydon/t3
	decor_quality = 3

/// Create headstone and gravefence if desired
/obj/structure/closet/dirthole/closed/Initialize()
	// Set up and spawn headstone
	if(spawn_headstone)
		//if headstone_spawn_list is empty, we generate it ourselves..
		if(!length(headstone_spawn_list))
			headstone_spawn_list = list()
			for(var/obj/item/gravedecor/headstone/potential_head as anything in typesof(/obj/item/gravedecor/headstone))
				var/quality = potential_head.decorationquality
				var/patronTEST = potential_head.patrons
				if(potential_head.decorationquality != decor_quality) // Too good/bad for them....
					continue
				else if(length(potential_head.patrons))
					var/failed = FALSE
					for(var/datum/P in potential_head.patrons)
						if(!(P.type in patrons_allowed)) // Not something this grave should have, wrong faith
							failed = TRUE
							break
					if(failed)
						continue
		...
		headstone = new new_headstone.type
