...
/datum/status_effect/jitter/on_remove()
	...
	owner.remove_stress(/datum/stress_event/jittery)
	owner.pixel_x = 0
	owner.pixel_y = 0
	return ..()
...
/datum/status_effect/jitter/get_examine_text()
	switch(duration)
		if(5 MINUTES to INFINITY)
			return span_boldwarning("SUBJECTPRONOUN is convulsing violently!")
		if(3 MINUTES to 5 MINUTES)
			return span_warning("SUBJECTPRONOUN is extremely jittery.")
		if(1 MINUTES to 3 MINUTES)
			return span_warning("SUBJECTPRONOUN is twitching ever so slightly.")
...
/datum/status_effect/jitter/tick(seconds_between_ticks)
	...
	owner.add_stress(/datum/stress_event/jittery)
