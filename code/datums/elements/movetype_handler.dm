...
/datum/element/movetype_handler/Attach(datum/target)
	...
	if(movable_target.movement_type & (MOVETYPES_NOT_TOUCHING_GROUND) && !HAS_TRAIT(movable_target, TRAIT_NO_FLOATING_ANIM))
...
/datum/element/movetype_handler/proc/on_movement_type_trait_gain(atom/movable/source, trait)
	...
	if(!(old_state & (MOVETYPES_NOT_TOUCHING_GROUND)) && (source.movement_type & (MOVETYPES_NOT_TOUCHING_GROUND)) && !HAS_TRAIT(source, TRAIT_NO_FLOATING_ANIM))
