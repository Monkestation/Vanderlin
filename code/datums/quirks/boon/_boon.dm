...
/datum/quirk/boon/always_prepared/after_job_spawn(datum/job/job)
	...
	var/obj/item/flashlight/flare/torch/lantern/lantern = new(T)
	var/obj/item/tent_kit/tent = new(T)

	if(customization_value == "With Cart")
		var/obj/structure/handcart/cart = new(T)
		cart.put_in(null, lantern)
		cart.put_in(null, tent)
	else
		if(!owner.equip_to_appropriate_slot(lantern) || isturf(lantern.loc)) //missing a limb can cause phantom success procs
			for(var/atom/storage in owner.contents)
				if(storage.atom_storage.attempt_insert(lantern, override = TRUE))
					break
		if(!owner.equip_to_appropriate_slot(tent)|| isturf(tent.loc))
			for(var/atom/storage in owner.contents)
				if(storage.atom_storage.attempt_insert(tent, override = TRUE))
					break
