...
/mob/living/proc/calculate_parry_values(obj/item/mainhand, obj/item/offhand)
	...
	if(mainhand && mainhand.can_parry)
		mainhand_defense += (skills ? (get_skill_level(mainhand.associated_skill, TRUE) * 20) : 20)
		mainhand_defense += (mainhand.wdefense * 10)

	if(offhand && offhand.can_parry)
		offhand_defense += (skills ? (get_skill_level(offhand.associated_skill, TRUE) * 20) : 20)
	...
	var/unarmed_defense = skills ? (get_skill_level(/datum/skill/combat/unarmed, TRUE) * 20) : 20
