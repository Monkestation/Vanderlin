...
/datum/organ_process/brain/handle_process(mob/living/carbon/owner, seconds_per_tick)
	...
	switch(effective_blood_oxygenation)
		if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
			...
			// owner.adjustOxyLoss(round(0.005 * (BLOOD_VOLUME_NORMAL - effective_blood_oxygenation) * seconds_per_tick * 0.3, 1))
			damprob = is_stable ? 10 : 30
			if((brain.current_blood <= 0) && !brain.past_damage_threshold(2) && SPT_PROB(damprob, seconds_per_tick))
		...
		if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_OKAY)
			...
			// owner.adjustOxyLoss(FLOOR(0.01 * (BLOOD_VOLUME_NORMAL - effective_blood_oxygenation) * seconds_per_tick * 0.3, 1))
			damprob = is_stable ? 15 : 45
			if((brain.current_blood <= 0) && !brain.past_damage_threshold(4) && SPT_PROB(damprob, seconds_per_tick))
				brain.applyOrganDamage(BRAIN_DAMAGE_LOW_OXYGENATION)
		if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
			owner.adjust_eye_blur_up_to(6, 6)
			if(SPT_PROB(2.5, seconds_per_tick))
				to_chat(owner, span_userdanger("<i>I feel extremely [pick("dizzy","woozy","faint")]...</i>"))
			// owner.adjustOxyLoss(2.5 * seconds_per_tick)
			damprob = is_stable ? 25 : 75
			if((brain.current_blood <= 0) && !brain.past_damage_threshold(6) && SPT_PROB(damprob, seconds_per_tick))
